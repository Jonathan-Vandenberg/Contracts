...
describe("Grants management", function () {
  ...
  beforeEach(async () => {
    ({
      ...
      user1,
      user2,
    } = await loadFixture(deployProjectFactoryWithProjectAndPool));
  });
  ...
  describe("Adding new grants", function () {
    it("add new grants by a manager should succeed", async function () {
      ...
      await expect(project.addGrants(0, [user2.address], [amount]))
        .to.emit(project, "GrantAdded")
        .withArgs(0, owner.address, user2.address, amount);
    ...
    it("user without manager role adding should fail", async function () {
      await expect(project.connect(user1).addGrants(0, [], [])).to.rejectedWith(
        "ManageableUpgradeable::onlyManager: the caller is not an manager."
      );
    });

    it("no grantees should fail", async function () {
      await expect(project.addGrants(0, [], [])).to.rejectedWith(
    ...
    it("different quantity of grantees and amounts should fail", async function () {
      await expect(project.addGrants(0, [user1.address], [])).to.rejectedWith(
        "LinearVestingProject::addTokenGrants: invalid parameters length (they should be same)"
      );
    });

    it("invalid vesting index should fail", async function () {
      await expect(
  ...
  describe("Remove existing grants", function () {
    it("remove existing grants by a manager should succeed", async function () {
      ...
      await expect(project.removeGrant(0, user1.address))
        .to.emit(project, "GrantRemoved")
    ...
    it("invalid grant or without funds available should fail", async function () {
      await expect(project.removeGrant(0, user2.address)).to.rejectedWith(
        "LinearVestingProjectRemovable::removeGrant: grant not active"
      );
    });

    it("invalid pool index should fail", async function () {
      await expect(project.removeGrant(1, user1.address)).to.rejectedWith(
        "LinearVestingProjectRemovable::removeGrant: grant not active"
      );
    });
  });
